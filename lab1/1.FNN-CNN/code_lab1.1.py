...
plt.savefig('results/1.1/model_accuracy.pdf')
plt.close()
#Loss plot
...
plt.xlabel('epoch')
plt.legend(['train'], loc='upper left')
plt.savefig('results/1.1/model_loss.pdf')
...
nn_json = nn.to_json()
with open('results/1.1/nn.json', 'w') as json_file:
    json_file.write(nn_json)
weights_file = "results/1.1/weights-MNIST_"+str(score[1])+".hdf5"
nn.save_weights(weights_file,overwrite=True)

#Loading model and weights
json_file = open('results/1.1/nn.json','r')
